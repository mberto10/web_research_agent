        ...
        "prompts": {
            ...
            "nodes": {
                ...
                "finalize_react_writer": (
                    ...
                    "2. Cite sources using markdown links: [descriptive text](url) - these will be automatically numbered\n"
                    "3. Do NOT create a Sources section - it will be automatically generated\n"
                    "4. Do NOT add any offers or follow-up questions\n"
                    "5. End the report immediately after your last content section\n\n"
                    "CITATION EXAMPLES:\n"
                    "✅ Correct: Recent [AI research](https://example.com/ai-study) shows improvements\n"
                    "✅ Correct: The market grew 15% according to [Q4 earnings](https://example.com/q4)\n"
                    "❌ Wrong: Recent research [1] shows improvements\n"
                    "❌ Wrong: According to research (https://example.com/study)\n"
                    "❌ Wrong: Creating a ## Sources section at the end"
                ),
                "llm_analyzer_system": (
...
def _load_config_from_yaml() -> Dict[str, Any]:
    """Load configuration from config/settings.yaml (or defaults if missing)."""
    base = _default_config()
    settings_path = Path("config/settings.yaml")
    if not yaml or not settings_path.exists():
        return base
    try:
        with open(settings_path, "r", encoding="utf-8") as f:
            settings = yaml.safe_load(f) or {}
        _deep_merge(base, settings)
        logger.info("✓ Loaded configuration from config/settings.yaml")
    except Exception as exc:
        logger.warning(f"⚠️ Failed to load config/settings.yaml, using defaults: {exc}")
    return base
...
def load_config() -> Dict[str, Any]:
    ...
    # No DB config yet—fall back to YAML/default settings
    _CONFIG_CACHE = _load_config_from_yaml()
    return _CONFIG_CACHE
